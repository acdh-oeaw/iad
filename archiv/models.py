...
class AltName(IdProvider):
    label = models.CharField(
        blank=True, null=True, max_length=250, verbose_name="A name tag"
    )
    language = models.CharField(
        blank=True, null=True, max_length=3, verbose_name="ISO639"
    )
    ...
    def get_absolute_url(self):
        return reverse("archiv:altname_detail", kwargs={"pk": self.id})
...
class IadBaseClass(IdProvider):
    ...
    def get_points(self):
        ...
        ct = ContentType.objects.get(
            app_label="archiv", model=model_name.lower()
        ).model_class()
        geojson = serialize(
            "geojson",
            ct.objects.all(),
    ...
    def get_convex_hull(self):
        if Site.objects.exclude(polygon=None):
            sites = [
                x.id for x in Site.objects.exclude(polygon=None) if x.polygon.valid
            ]
...
class Period(IadBaseClass):
    ...
    norm_id = models.CharField(
        blank=True, null=True, max_length=250, verbose_name="PeriodO"
    )
    ...
    def get_geojson(self):
        geojson = serialize(
            "geojson",
            Period.objects.filter(id=self.id),
            geometry_field="polygon",
            fields=(
                "name",
                "identifier",
...
SITE_POTENTIALSURROUNDINGS = (
    ...
    ("2 – touristic offer in development", "2 – touristic offer in development"),
    ("3 – no or little attempts for tourism", "3 – no or little attempts for tourism"),
    ("4 – information not available", "4 – information not available"),
)
...
class Site(IadBaseClass):
    ...
    def get_geojson(self):
        geojson = serialize(
            "geojson",
            Site.objects.filter(id=self.id),
            geometry_field="polygon",
    ...
    def get_next_public(self):
        next = (
            Site.objects.filter(id__gt=self.id)
            .exclude(site_checked_by=None)
    ...
    def get_prev_public(self):
        prev = (
            Site.objects.filter(id__lt=self.id)
            .exclude(site_checked_by=None)
            .exclude(public=False)
            .order_by("-id")
        )
...
class ResearchQuestion(IdProvider):
    ...
    def get_absolute_url(self):
        return reverse("archiv:researchquestion_detail", kwargs={"pk": self.id})
...
class ResearchEvent(IadBaseClass):
    ...
    def get_geojson(self):
        geojson = serialize(
            "geojson",
            ResearchEvent.objects.filter(id=self.id),
            geometry_field="polygon",
            fields=(
                "name",
                "identifier",
    ...
    def get_site_geojson(self):
        if self.site_id:
            geojson = serialize(
                "geojson",
                Site.objects.filter(has_research_activity__in=[self.id]),
                geometry_field="polygon",
                fields=(
                    "name",
                    "identifier",
    ...
    def get_absolute_url(self):
        return reverse("archiv:researchevent_detail", kwargs={"pk": self.id})
    ...
    def convex_hulls(self):
        if self.site_id.exclude(polygon=None):
            ...
            geojson["properties"] = {"name": "Convex hull of all related Sites"}
            geojson = json.dumps(geojson)
            return geojson
        else:
            return None
...
class ArchEnt(IadBaseClass):
    ...
    def get_site_geojson(self):
        if self.site_id:
            geojson = serialize(
                "geojson",
                Site.objects.filter(has_archent__in=[self.id]),
                geometry_field="polygon",
                fields=(
                    "name",
                    "identifier",
    ...
    def get_geojson(self):
        geojson = serialize(
            "geojson",
            ArchEnt.objects.filter(id=self.id),
            geometry_field="polygon",
            fields=("name", "identifier", "pk"),
        )
        return geojson
    ...
    def get_absolute_url(self):
        return reverse("archiv:archent_detail", kwargs={"pk": self.id})
...
class MonumentProtection(IadBaseClass):
    ...
    def get_geojson(self):
        geojson = serialize(
            "geojson",
            MonumentProtection.objects.filter(id=self.id),
            geometry_field="polygon",
            fields=(
                "name",
                "identifier",
    ...
    def get_site_geojson(self):
        if self.site_id:
            geojson = serialize(
                "geojson",
                Site.objects.filter(has_monument_protection__in=[self.id]),
                geometry_field="polygon",
                fields=(
                    "name",
                    "identifier",
