...
class Csv2SkosReader(object):
    ...
    def __init__(self, csv_file):
        ...
        try:
            self.alt_lang = (self.headers[1])[(self.headers[1]).index("@") + 1 :]  # noqa
        except:  # noqa: E722
            self.alt_lang = None
    ...
    def returnDescriptions(self):
        ...
        for x in self.extractedDescriptions:
            ...
            if temp_type is not None:
                description["type"] = temp_type.attrib[
                    "{http://www.w3.org/1999/02/22-rdf-syntax-ns#}resource"
                ]
            else:
                description["type"] = "no type"
            description["id"] = x.attrib[
                "{http://www.w3.org/1999/02/22-rdf-syntax-ns#}about"
            ]
            description["notation"] = x.find(
                "skos:notation", namespaces={"skos": self.ns_skos}
            )
    ...
    def importConcepts(self):
        ...
        for x in self.returnDescriptions():
            if x["type"] == "http://www.w3.org/2004/02/skos/core#ConceptScheme":
                temp_concept_scheme, _ = SkosConceptScheme.objects.get_or_create(
                    legacy_id=x["id"]
                )
